...
| 37 | **[ITERACIÓN VISUAL — 2026-03-14]** Calidad estética insuficiente en selects, modales y canvas. | `index.css`, `App.css`, `ZoneEditorModal.jsx`, `ZoneCanvas.jsx`, `ZoneRect.jsx`, `ZoneToolbar.jsx`, `ItemDialog.jsx`, `ItemDialog.css`, `fzUtils.jsx` | **SELECT premium global**: `appearance:none` + `-webkit-appearance:none` + custom chevron SVG cyan `stroke-width:2.5`, `background-position:right 13px`, transición `border-color/background/box-shadow/transform`, `optgroup` styling (oscuro `#111113`, uppercase 10px, tracking). **Modal ZoneEditorModal**: overlay `rgba(0,0,0,0.88) blur(20px) saturate(140%)` + `animation:fzOverlayIn 0.22s`; dialog `fzDialogIn: scale(0.88)→1 translateY(20px)→0 blur(5px)→0 cubic-bezier(0.34,1.56,0.64,1) 0.32s`; header con icon-box 38px glassmorphism + accent border; label markers (barra vertical 3px accent); spacing pill `rgba(0,210,255,0.1)` monospace; range slider thumb 18px con `cubic-bezier(0.34,1.56)` spring. **Modal ItemDialog**: overlay `blur(18px) saturate(150%)`, dialog `rgba(18,18,20,0.97)` borde inset + top gradient line, `animation:dialogIn 0.32s cubic-bezier(0.34,1.56)`, close btn con rotación 6deg en hover; type-grid shimmer + scrim gradient on hover; labels con barrita vertical accent 3px; `.fz-visualizer` glassmorphism panel; `.btn-launch` con gradiente inset + glow hover; footer glassmorphism bg. **ZoneCanvas**: split-preview line reemplazada por `linear-gradient(90deg,transparent,var(--fz-accent),transparent)` con `boxShadow` glow; HUD split-mode indicator flotante (icon mini + texto) glassmorphism en bottom-center; prop `occupancyMap` pass-through a `ZoneRect`. **ZoneRect**: new `OccupancyBadge` component — chip colapsado (dot verde pulsante + contador pills) vs expandido (glassmorphism cascade `rgba(0,0,0,0.7) blur(14px)` con colored dots por app `OCCUPANT_COLORS[]`); `occupancyApps[]` prop; background dinámico linear-gradient según estado (selected/hover/hasApps/idle); border y boxShadow 3-estado; shimmer sweep on hover; number font-size adaptativo `Math.min(64,Math.max(28,…))`; pixel dims con `backdropFilter:blur(4px)`. **ZoneToolbar**: glassmorphism container; preset buttons con hover JS inline `rgba(0,210,255,0.12)`, border cyan, `translateY(-1px)`, reset btn con hover css danger-red. **fzUtils.jsx**: reemplazado `<button>` plano por `ZonePreviewCell` — 4 estados de fondo (`isSelected/hovered/hasOccupants/idle`) con `linear-gradient`; dots dots-collapse vs cascade chips expand `chipFadeIn/chipSlide` @ hover/isSelected; `OCCUPANT_COLORS[]` palette 6 colores; glow radial en selected; `transform:scale(0.97/0.96)` interactivo; tooltip `title` detallado. |
